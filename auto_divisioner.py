...
class AutoDivisionerThread(threading.Thread):
    ...
    def _execute(self) -> tuple[bool, str]:
        ...
        log.append(f"Step 1: Initial position: X={x0:.4f}, Y={y0:.4f} um")
...
##        self.master.reverse_axis1_var.set(reverse_axis1)
##        self.master.reverse_axis2_var.set(reverse_axis2)
##        self.master.swap_axes_var.set(swap_axes)
        ...
        if self.master.swap_axes:
            ch = 2 if ch == 1 else 1
        if ch == 1 and self.master.reverse_axis1:
            pulses = -pulses
        elif ch == 2 and self.master.reverse_axis2:
            pulses = -pulses


        
        if not self._drive_and_wait(ch, pulses):
        ...
        log.append(f"Step 8: X diff = {dx_verify:.4f} um -> {'OK' if step8_ok else 'FAIL'}")
        print(f"[AutoDiv] {log[-1]}")
        ...
        if abs(dy_verify) > 0.001:
            ...
            log.append(f"Pulse calibration (Y/Axis1): {cal_y:.1f} pulses/um")
            print(f"[AutoDiv] {log[-1]}")
        if abs(dx_verify) > 0.001:
            cal_x = DRIVE_PULSES / abs(dx_verify)
            cal_values.append(cal_x)
            log.append(f"Pulse calibration (X/Axis2): {cal_x:.1f} pulses/um")
            print(f"[AutoDiv] {log[-1]}")
        ...
        if cal_values:
            ...
            log.append(f"Pulse calibration applied to ADC: {cal_int} pulses/um")
            print(f"[AutoDiv] {log[-1]}")
        ...
        if step7_ok and step8_ok:
            ...
            if abs(dy_verify) > 0.001:
                summary += f"\nPulse Cal (Axis1/Y): {DRIVE_PULSES / abs(dy_verify):.1f} pulses/um"
            if abs(dx_verify) > 0.001:
                summary += f"\nPulse Cal (Axis2/X): {DRIVE_PULSES / abs(dx_verify):.1f} pulses/um"
            return True, summary
        else:
            errors = []
            if not step7_ok:
