...
##        self.units_box.bind("<<ComboboxSelected>>", self._set_units)
        ...
        distance_frame = tk.Frame(self.ac_frame, padx=20, pady=10)
        distance_label = tk.Label(distance_frame, text="Distance to sensor (mm):")
        distance_label.pack(side=tk.LEFT)
        self.distance_box = tk.Entry(distance_frame, width=10)
        self.distance_box.pack(side=tk.LEFT, padx=5)
        self.distance_box.insert(tk.END, "50")
        self.current_distance=int(self.distance_box.get())
        start_btn = tk.Button(distance_frame, text="Start Reading", command=self.start_reading, width=15)
        stop_btn  = tk.Button(distance_frame, text="Stop Reading",  command=self.stop_reading,  width=15)
...
##        stop_btn  = tk.Button(control_frame, text="Stop Reading",  command=self.stop_reading,  width=15)
        ...
        self._draw_cross()
        self.xvar = self.aln_canvas.create_text(60, 330, anchor="w", text="X-Pos: 0.0000 um", fill="white", font=("Arial", 12))
        self.xvarangle = self.aln_canvas.create_text(60, 345, anchor="w", text="X-Ang: 0.0000 mrad", fill="white", font=("Arial", 12))
        self.yvar = self.aln_canvas.create_text(60, 375, anchor="w", text="Y-Pos: 0.0000 um", fill="white", font=("Arial", 12))
        self.yvarangle = self.aln_canvas.create_text(60, 390, anchor="w", text="Y-Ang: 0.0000 mrad", fill="white", font=("Arial", 12))
        self.xline = self.aln_canvas.create_line(220, 230, 240, 230, fill="red",   width=3)
        self.yline = self.aln_canvas.create_line(230, 220, 230, 240, fill="red",   width=3)
        self.target_xline = self.aln_canvas.create_line(220, 230, 240, 230, fill="green", width=3)
        self.target_yline = self.aln_canvas.create_line(230, 220, 230, 240, fill="green", width=3)
        self.range_circle = self.aln_canvas.create_oval(215, 215, 245, 245, fill="", outline="red", width=2)
    ...
    def _setup_ADC_tab(self):
        ...
        self.error_x_label = tk.Label(error_frame, text="X Error: 0.00 um", font=("Arial", 10))
        self.error_y_label = tk.Label(error_frame, text="Y Error: 0.00 um", font=("Arial", 10))
...
##        self.save_data_btn.pack(pady=5)
        self.start_test_btn.pack(side=tk.LEFT, pady=5)
        self.stop_test_btn.pack(side=tk.LEFT, pady=5)
        self.save_data_btn.pack(side=tk.LEFT, pady=5)
        ...
        self.test_status_label.pack(side=tk.LEFT,pady=10)
        self.data_count_label.pack(side=tk.LEFT,pady=5)
...
##        self.data_count_label.pack(pady=5)
        ...
        tk.Label(sample_frame, text="Sampling Frequency", font=("Arial", 10, "bold")).pack(side=tk.LEFT,pady=5)
        tk.Label(sample_frame, text="Sample Period (s):").pack(side=tk.LEFT,anchor="w", pady=(5, 0))
        self.logging_sample_period_var = tk.StringVar(value="0.1")
        tk.Entry(sample_frame, textvariable=self.logging_sample_period_var, width=10).pack(side=tk.LEFT,anchor="w", pady=2)
...
##        tk.Entry(sample_frame, textvariable=self.logging_sample_period_var, width=10).pack(anchor="w", pady=2)
    ...
    def _draw_cross(self):
        ...
        # Scale: 20 units (-10 to +10)
        unit = GRID / 20   # = 20 px per µm
        ...
        for i in range(-10, 11):
            ...
            # Vertical grid line
            canvas.create_line(x, M, x, M + GRID, fill="#333333")

            # Horizontal grid line
            canvas.create_line(M, y, M + GRID, y, fill="#333333")

        # Draw main axes
        canvas.create_line(cx, M, cx, M + GRID, fill="white", width=2)   # Y axis
        canvas.create_line(M, cy, M + GRID, cy, fill="white", width=2)   # X axis
        ...
        for i in range(-10, 11):
            ...
            if i in [-10, -5, 0, 5, 10]:
                ...
                tick_len = 5  # Minor ticks (1 µm)

            # X-axis ticks
            canvas.create_line(x, cy - tick_len, x, cy + tick_len, fill="white")

            # Y-axis ticks
            canvas.create_line(cx - tick_len, y, cx + tick_len, y, fill="white")

            # Labels only for major ticks
            if i in [-10, -5, 0, 5, 10]:
                if i != 0:
                    # X-axis labels (below grid, inside canvas)
                    canvas.create_text(x, M + GRID + 15, text=str(i),
                                       fill="white", font=("Arial", 10))

                    # Y-axis labels (left of grid, inside canvas)
                    canvas.create_text(M - 15, y, text=str(-i),
                                       fill="white", font=("Arial", 10))

        # Axis labels (inside canvas margins)
        canvas.create_text(M + GRID + 30, cy, text="X (µm)", fill="white", font=("Arial", 12))
        canvas.create_text(cx, M - 20, text="Y (µm)", fill="white", font=("Arial", 12))
...
##                self.current_unit = self.units_box.get()
    ...
    def update_display(self):
        ...
        if self.xinrange:
            self.aln_canvas.itemconfig(self.xvar, text=f"X-Pos: {display_x:.4f} um")
            self.aln_canvas.itemconfig(self.xvarangle, text=f"X-Ang: {rad_x:.4f} mrad")
        else:
            self.aln_canvas.itemconfig(self.xvar, text=f"X-Pos: Out of range")
            self.aln_canvas.itemconfig(self.xvarangle, text=f"X-Ang: Out of range")
        if self.yinrange:        
            self.aln_canvas.itemconfig(self.yvar, text=f"Y-Pos: {display_y:.4f} um")
            self.aln_canvas.itemconfig(self.yvarangle, text=f"Y-Ang: {rad_y:.4f} mrad")
        else:
            self.aln_canvas.itemconfig(self.yvar, text=f"Y-Pos:  Out of range")
            self.aln_canvas.itemconfig(self.yvarangle, text=f"Y-Ang: Out of range")

        scale = 18
        xo = display_x * scale
        yo = display_y * scale
        self.aln_canvas.coords(self.xline, 230 + xo - 10, 230 - yo, 230 + xo + 10, 230 - yo)
        self.aln_canvas.coords(self.yline, 230 + xo, 230 - yo - 10, 230 + xo, 230 - yo + 10)
        try:
            range_thr = float(self.ADC_convergence_threshold_var.get())*18
            self.aln_canvas.coords(self.range_circle, 230 - range_thr, 230 - range_thr, 230 + range_thr, 230 + range_thr)
        except:
            pass
    ...
    def update_target_reticle(self):
        ...
        self.aln_canvas.coords(self.target_xline, 230 + xo - 10, 230 - yo, 230 + xo + 10, 230 - yo)
        self.aln_canvas.coords(self.target_yline, 230 + xo, 230 - yo - 10, 230 + xo, 230 - yo + 10)

    def update_ADC_display(self):
        self.error_x_label.config(text=f"X Error: {self.ADC_error_x:.4f} um")
        self.error_y_label.config(text=f"Y Error: {self.ADC_error_y:.4f} um")
...
##            return
    ...
    def start_ADC(self):
        ...
        if not self.worker or not self.worker.running:
            messagebox.showerror("Error", "Please start reading from autocollimator first.")
            return
        if not (self.xinrange or self.yinrange):
            messagebox.showerror("Error", "Please make sure the reading from autocollimator is in range first.")
            return
        ...
        if filename:
            try:
                with open(filename, "w", newline="") as f:
                    f.write("Time(s),X-pos(um),Y-pos(um),X-Ang(mrad),Y-Ang(mrad), X-pos target(um), Y-pos target(um), X-pos error(um), Y-pos error(um), Stabilization state\n")
                    for entry in self.logged_data:
                        f.write(f"{entry['elapsed_time']:.3f},{entry['alnx']:.2f},{entry['alny']:.2f},{entry['angx']:.3f},{entry['angy']:.3f},{self.ADC_target_x},{self.ADC_target_y},{entry['errorx']:.4f},{entry['errory']:.4f},{entry['adc_stat']:.0f}\n")
